...
# -*- coding: utf-8 -*-
"""
Segmentación interactiva con SAM usando puntos positivos y negativos.
Puede ejecutarse directamente o importarse desde otro script.

Uso directo:
    python segment_sam_points.py [imagen_path]

Uso como módulo:
    from segment_sam_points import segment_image
    mask, score = segment_image("path/to/image.png")
"""
...
DEFAULT_CHECKPOINT = "/Users/thomasmolinamolina/Downloads/UNAL/MATERIAS/SEMESTRE 6/PALUZNY/Checkpoints/sam_vit_h_4b8939.pth"
DEFAULT_IMAGE = "/Users/thomasmolinamolina/Downloads/UNAL/MATERIAS/SEMESTRE 6/PALUZNY/DATA/D1/pngs/I15.png"
...
def load_sam_model(checkpoint=DEFAULT_CHECKPOINT, model_type="vit_h", device=None):
    """Carga el modelo SAM"""
    if device is None:
        device = get_device()
    
    print(f"🔄 Cargando modelo SAM ({model_type})...")
...
def segment_image(image_path, checkpoint=DEFAULT_CHECKPOINT, model_type="vit_h", 
                  output_dir=DEFAULT_OUTPUT_DIR, show_visualization=True):
    """
    Función principal para segmentar una imagen interactivamente.
    
    Args:
        image_path: Ruta a la imagen a segmentar
        checkpoint: Ruta al checkpoint de SAM
        model_type: Tipo de modelo SAM ("vit_h", "vit_l", "vit_b")
        output_dir: Directorio donde guardar los resultados
        show_visualization: Si mostrar la visualización final
    
    Returns:
        tuple: (refined_mask, best_score, positive_points, negative_points)
    """
    device = get_device()
    
    # Cargar modelo
    predictor, device = load_sam_model(checkpoint, model_type, device)
    
    # Cargar imagen
    print(f"📂 Cargando imagen: {image_path}")
    img = np.array(Image.open(image_path).convert("RGB"))
    
    # Mejorar contraste para imágenes médicas
    ...
    if len(input_points) == 0:
        print("⚠️ No se seleccionaron puntos.")
        return None, 0.0, [], []
    
    input_points = np.array(input_points)
    input_labels = np.array(input_labels)
    
    print(f"\n✅ Total de puntos: {len(input_points)}")
    ...
    return refined_mask, best_score, positive_points, negative_points


def main():
    """Función principal para ejecución desde línea de comandos"""
    parser = argparse.ArgumentParser(
        description="Segmentación interactiva con SAM usando puntos positivos y negativos"
    )
    parser.add_argument(
        "image_path", 
        nargs="?", 
        default=DEFAULT_IMAGE,
        help=f"Ruta a la imagen a segmentar (default: {DEFAULT_IMAGE})"
    )
    parser.add_argument(
        "--checkpoint", "-c",
        default=DEFAULT_CHECKPOINT,
        help="Ruta al checkpoint de SAM"
    )
    parser.add_argument(
        "--model", "-m",
        default="vit_h",
        choices=["vit_h", "vit_l", "vit_b"],
        help="Tipo de modelo SAM (default: vit_h)"
    )
    parser.add_argument(
        "--output", "-o",
    ...
    parser.add_argument(
        "--no-viz",
        action="store_true",
        help="No mostrar visualización final"
    )
    ...
    mask, score, pos_pts, neg_pts = segment_image(
    ...
    if mask is None:
        print("❌ Segmentación cancelada o fallida")
        sys.exit(1)
